...
LDexcludeclean <- dplyr::select(excludecorrections, CTW_added_manually:NBD_Notes, exclude_LD, Reason_LD, ResponseId, Title, Init, exclude_notes, SurveyNotes) %>%
  ...
  left_join(distinct(prelimlong1b[c("Title", "Init", "ResponseId")]), by = c("Title", "ResponseId")) %>%
  mutate(Init = ifelse(is.na(Init.x), Init.y, Init.x),
         outsidereviewer = ifelse(!is.na(NBD_Notes), "NBD", ifelse(CTW_added_manually == "Yes", "CTW", "LD")),
         # clean up SurveyNotes
         SurveyNotes = gsub("didn’t", "didn't", SurveyNotes),
         SurveyNotes = trimws(SurveyNotes),
         NBD_Notes = gsub("abstrcr", "abstract", NBD_Notes),
         NBD_Notes = gsub("Nick et als", "NBD and AK", NBD_Notes),
...
excludedv2 <- subset(prelimlong1b, Title %in% unique(with(prelimlong1b, Title[ResponseId %in% c(exclude_qualtrics, excludeLD)]))) %>%
  ...
  mutate(doublerev = Title %in% doubletitles,
         ...
         version = ifelse(!doublerev, "final", "original"),
  ...
  group_by(ResponseId) %>%
  mutate(clean_answer = ifelse(any(unique(answer) == "Yes") & is.na(answer), "NA (paper already excluded)", answer),
         clean_answer = ifelse(all(unique(answer[!is.na(answer)]) == "No") & is.na(answer), "No answer (question not yet created)", clean_answer)) %>%
  ungroup() %>%
...
for(t in unique(dblcor_all$Title)){
  ...
  tempdat <- subset(dblcor_all, Title == t) %>%
    data.frame() %>%
    arrange(revorder, survey_order, varnum)
  ...
  for(s in unique(tempdat$survey_order)){
    ...
    if(any(!is.na(tempanswers$new))){
      ...
      if(!grepl("Response|Driver", unique(tempanswers$abbr))){
        ...
        # pull responses and drivers [in case correction made to clean_answer above and not reflected in answer col]
        if(with(tempanswers[tempanswers$revorder == 1,], all(is.na(answer)) & !all(is.na(clean_answer)))){
          rev1tempanswer <- with(tempanswers[tempanswers$revorder == 1,], str_flatten(clean_answer, collapse = ", "))
          # replace for code below
          tempanswers$answer[tempanswers$revorder == 1 & is.na(tempanswers$answer)] <- rev1tempanswer
        } 
        if(with(tempanswers[tempanswers$revorder == 2,], all(is.na(answer)) & !all(is.na(clean_answer)))){
          rev2tempanswer <- with(tempanswers[tempanswers$revorder == 2,], str_flatten(clean_answer, collapse = ", "))
          # replace for code below
          tempanswers$answer[tempanswers$revorder == 2 & is.na(tempanswers$answer)] <- rev2tempanswer
        }
...
dblidcheck <- select(dblcor_clean, c(Init, Title, names(idcheck[names(idcheck) != "check"]))) %>%
  left_join(idcheck)
